normalise_raster <- function(x){(x-terra::minmax(dem)[1])/(terra::minmax(dem)[2]-terra::minmax(dem)[1])}
...
compare_lcps <- function(cs_list, cfs, cf1_index, cf2_index, pts) {
  ...
  pdi <- leastcostpath::PDI_validation(lcp = lcp1, comparison = lcp2)
  
  lcp1$pts_dist <- pdi$max_distance
  lcp1$pdi <- pdi$pdi
  lcp1$normalised_pdi <- pdi$normalised_pdi
